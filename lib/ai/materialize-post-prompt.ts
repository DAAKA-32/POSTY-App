...
- FINAL CHECKLIST (non-negotiable — these are often skipped): (a) use emojis exactly per the EMOJIS policy above — contextual, varied, an editorial tool (0 is fine when none earns its place, never forced); (b) if the AUTHOR VOICE block defines a personalized signature, END with it; (c) close on exactly ONE open question OR CTA; (d) 2-3 camelCase hashtags on the last line, always ending with #posty.
...
- CHECKLIST FINALE (non négociable — souvent oubliée) : (a) utilise les emojis selon la POLITIQUE EMOJIS ci-dessus — contextuels, variés, comme un outil éditorial (0 est acceptable si aucun ne s'impose, jamais forcé) ; (b) si le bloc VOIX DE L'AUTEUR définit une signature personnalisée, TERMINE par elle ; (c) clôture sur UNE seule question ouverte OU UN CTA ; (d) 2 à 3 hashtags camelCase sur la dernière ligne, en terminant toujours par #posty.
