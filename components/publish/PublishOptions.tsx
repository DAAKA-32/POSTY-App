  ...
  return (
    <div className="space-y-3">
      ...
      <div className="flex items-center justify-between">
        ...
        {!hasSelection && (
          <span className="text-xs text-error">Sélectionnez au moins une option</span>
        )}
      </div>
            ...
            <div className="flex-1">
              <div className="flex items-center gap-2 mb-1">
                ...
                <h4 className={`font-semibold ${publishToFeed ? 'text-white' : 'text-text-secondary'}`}>
                  Publier en post LinkedIn
                </h4>
              </div>
              <p className="text-sm text-text-muted mb-2">
                Votre contenu sera visible publiquement sur votre profil
              </p>
              <div className="flex items-center gap-2 text-xs">
                <div className="flex items-center gap-1 px-2 py-1 bg-dark-bg rounded-md text-text-muted">
                  ...
                  <span>Portée estimée: ~{estimatedReach.toLocaleString()} personnes</span>
                </div>
              </div>
            </div>
          </div>
            ...
            <div className="flex-1">
              <div className="flex items-center gap-2 mb-1">
                ...
                <h4 className={`font-semibold ${sendToConnections ? 'text-white' : 'text-text-secondary'}`}>
                  Envoyer en messages privés
                </h4>
              </div>
              <p className="text-sm text-text-muted mb-2">
                Envoyé directement à vos connexions LinkedIn
              </p>
              <div className="flex items-center gap-2 text-xs">
                <div className="flex items-center gap-1 px-2 py-1 bg-dark-bg rounded-md text-text-muted">
                  <svg className="w-3 h-3" fill="currentColor" viewBox="0 0 20 20">
                  ...
                  <span>
                    {connectionsCount > 0
                      ? `${connectionsCount.toLocaleString()} destinataires`
                      : 'Chargement...'}
                  </span>
                </div>
              </div>
              ...
              {sendToConnections && connectionsCount > 100 && (
                ...
                >
                  ...
                  <span>L'envoi de plus de 100 messages peut prendre quelques minutes</span>
                </motion.div>
              )}
            </div>
          </div>
      ...
      {publishToFeed && sendToConnections && (
        ...
        >
          ...
          <span className="text-text-secondary">
            Votre contenu sera <span className="text-white font-medium">publié publiquement</span> et{' '}
            <span className="text-white font-medium">envoyé à {connectionsCount} connexions</span>
          </span>
        </motion.div>
      )}
    </div>
