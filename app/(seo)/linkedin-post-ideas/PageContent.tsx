  ...
  return (
    <>
      ...
      <BreadcrumbJsonLd
        items={[
          { name: "Home", url: "/" },
          { name: t.breadcrumb, url: "/linkedin-post-ideas" },
        ]}
      />
      ...
      <article>
        ...
        {/* Idea Categories */}
        <section id="ideas" className="py-16 sm:py-24 bg-[#FAFBFC]">
          <div className="max-w-5xl mx-auto px-4 sm:px-6">
            <h2 className="text-3xl sm:text-4xl font-bold text-gray-900 text-center mb-4">
              {t.ideasSectionTitle}
            </h2>
            <p className="text-gray-600 text-center max-w-xl mx-auto mb-14">
              {t.ideasSectionSubtitle}
            </p>

            <div className="space-y-10">
              {t.ideaCategories.map((category, catIndex) => (
                <div key={catIndex} className="bg-white rounded-2xl border border-gray-200 shadow-sm p-6 sm:p-8">
                  ...
                  <ul className="space-y-3">
                    {category.ideas.map((idea, ideaIndex) => (
                      <li
                        key={ideaIndex}
                        className="flex items-start gap-3 bg-[#FFF8F5] rounded-xl p-4 border border-orange-100/50"
                      >
                        <span className="shrink-0 w-6 h-6 rounded-full bg-gradient-to-r from-[#F8935D] to-[#F76B54] text-white text-xs font-bold flex items-center justify-center mt-0.5">
                          {ideaIndex + 1}
                        </span>
        ...
        {/* How to Turn Ideas Into Posts */}
        <section className="py-16 sm:py-24 bg-white">
          <div className="max-w-4xl mx-auto px-4 sm:px-6">
            <h2 className="text-3xl sm:text-4xl font-bold text-gray-900 text-center mb-4">
              {t.turnIntoPost.title}
            </h2>
            <p className="text-gray-600 text-center max-w-xl mx-auto mb-14">
              {t.turnIntoPost.subtitle}
            </p>

            <div className="grid sm:grid-cols-2 gap-6">
              {t.turnIntoPost.steps.map((item) => (
                <div key={item.step} className="bg-[#FFF8F5] rounded-2xl border border-gray-200 shadow-sm p-6">
                  <div className="w-8 h-8 rounded-full bg-gradient-to-r from-[#F8935D] to-[#F76B54] text-white text-sm font-bold flex items-center justify-center mb-4">
                    {item.step}
                  </div>
